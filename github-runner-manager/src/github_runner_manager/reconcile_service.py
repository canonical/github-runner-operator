...
def get_runner_scaler(
    ...
    return RunnerScaler.build(app_config, user, python_path, reactive_log_dir)
...
def start_reconcile_service(
    ...
    resolved_state_dir = get_state_dir(state_dir)
    ...
    while True:
        with lock:
            ...
            runner_scaler = get_runner_scaler(
                app_config, python_path=python_path, reactive_log_dir=reactive_log_dir
            )
