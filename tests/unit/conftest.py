...
def mocks(monkeypatch, tmp_path, exec_command, lxd_exec_command, runner_binary_path):
    ...
    monkeypatch.setattr("lxd_cloud.event_timer.jinja2", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.event_timer.execute_command", exec_command)
    monkeypatch.setattr(
        "lxd_cloud.firewall.Firewall.get_host_ip", unittest.mock.MagicMock(return_value="10.0.0.1")
    )
    monkeypatch.setattr("lxd_cloud.firewall.Firewall.refresh_firewall", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.runner.execute_command", lxd_exec_command)
    monkeypatch.setattr("lxd_cloud.runner.shared_fs", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.metrics.execute_command", lxd_exec_command)
    monkeypatch.setattr("lxd_cloud.metrics.METRICS_LOG_PATH", Path(tmp_path / "metrics.log"))
    monkeypatch.setattr(
        "lxd_cloud.metrics.LOGROTATE_CONFIG", Path(tmp_path / "github-runner-metrics")
    )
    monkeypatch.setattr("lxd_cloud.runner.time", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.runner_manager_type.jinja2", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.runner_manager_type.LxdClient", MockLxdClient)
    monkeypatch.setattr("lxd_cloud.runner_manager.github_metrics", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.runner_manager.runner_logs", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.runner_manager.LxdClient", MockLxdClient)
    monkeypatch.setattr("lxd_cloud.runner_manager.shared_fs", unittest.mock.MagicMock())
    monkeypatch.setattr("lxd_cloud.runner_manager.execute_command", exec_command)
    monkeypatch.setattr(
        "lxd_cloud.runner_manager.RunnerManager.runner_bin_path", runner_binary_path
    )
    monkeypatch.setattr("lxd_cloud.runner_manager.RunnerManager.cron_path", cron_path)
    monkeypatch.setattr(
        "lxd_cloud.runner_manager.RepoPolicyComplianceClient", MockRepoPolicyComplianceClient
    )
